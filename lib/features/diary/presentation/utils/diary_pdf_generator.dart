...
class DiaryPdfGenerator {
  ...
  static Future<void> generateAndPrint({
    ...
    final pdf = await _createPdf(records: records, schoolClass: schoolClass, date: date);
  ...
  static Future<void> generateAndShareImage({
    ...
    await for (final image in images) {
      ...
      final file = File('${tempDir.path}/Diary_${schoolClass.name}_${DateFormat("dd-MM-yy").format(date)}.png');
      await file.writeAsBytes(pngBytes);
      
      await Share.shareXFiles(
        [XFile(file.path)],
        text: 'Daily Diary: ${schoolClass.name} - ${DateFormat("dd.MM.yy").format(date)}',
      );
      break; 
    }
  }
